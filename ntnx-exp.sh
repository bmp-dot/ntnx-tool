...
#updated	:2015-01-05
#version	:0.4    
#notes		:Only tested with NOS 4.0
...
SMCNEW=ADMIN
hostip="192.168.1.1 192.168.1.2 192.168.1.3"
...
for s in `ntnx-smcip`; do ipmitool -H $s -U ADMIN -P $SMCOLD user set password 2 $SMCNEW; done
...
for i in $hostip; do echo "Changing sshd_config on $i" && ssh root@$i 'echo "useDNS no" >> /etc/ssh/sshd_config';done ;;


5)
read -p "Enter new CVM VLAN Network tag: " CVMVLAN; 
for i in $hostip; do echo "Changing CVM VLAN Network tag on $i to $CVMVLAN" && ssh -f root@$i "nohup sleep 60 && esxcfg-vswitch -p 'VM Network' -v '$CVMVLAN' vSwitch0";done ;;


6)
read -p "Enter new ESX Management VLAN tag: " MGMTVLAN;
for i  in $hostip; do echo "Changing ESX Management VLAN tag on $i to $MGMTVLAN" && ssh -f root@$i "nohup sleep 60 && esxcfg-vswitch -p 'Management Network' -v '$MGMTVLAN' vSwitch0";done ;;


9)
for i in $hostip; do echo "Displaying VLANs on $i" && ssh root@$i "esxcfg-vswitch -l | grep 'VM Network' -B1 -A1";done ;;
